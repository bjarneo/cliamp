...
// savedAlbums returns the full saved-albums list (newest saved first),
// cached for playlistListCacheTTL.
func (p *SpotifyProvider) savedAlbums() ([]provider.AlbumInfo, error) {
	p.mu.Lock()
...
// AlbumTracks returns all tracks of an album. Simplified album-track objects
// lack album metadata, so one /v1/albums/{id} fetch supplies the album name,
// art URL, and release year that are merged into every track.
func (p *SpotifyProvider) AlbumTracks(albumID string) ([]playlist.Track, error) {
	...
	resp, err := p.webAPI(ctx, "GET", "/v1/albums/"+albumID, nil)
	if err != nil {
		return nil, fmt.Errorf("spotify: fetch album: %w", err)
	}
	var album spotifyAlbum
	if err := decodeBody(resp, &album); err != nil {
		return nil, fmt.Errorf("spotify: parse album: %w", err)
	}
	albumArtURL := ""
