	...
	return []providerSpec{
		...
		},
		{
			...
			intro: []string{
				...
				"",
				"Recommended: use the built-in client_id. No app registration.",
				"cliamp uses ncspot's Extended Quota Mode client_id for the Web",
				"API and the librespot keymaster for playback, so first use",
				"completes two steps in one browser tab. The quota is shared with",
				"other ncspot- and spotify-player-based clients, so you may see",
				"occasional 429 errors when the pool is busy.",
				"",
				"Advanced: register your own Spotify Developer app at",
				"developer.spotify.com/dashboard (redirect URI",
				"http://127.0.0.1:19872/login) for a private quota. Apps registered",
				"today start in Development Mode, which cannot read playlists you",
				"follow but do not own, and caps /v1/search at 10 results per",
				"request; cliamp pages search automatically with offset.",
			},
			picker: &pickerSpec{
				key:   keySpotifyMode,
				label: "Client ID",
				options: []pickerOption{
					{value: "default", label: "Use built-in client_id (recommended)"},
