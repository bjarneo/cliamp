...
// nothing is loaded, or after a start failed, reports nothing. A gapless
// boundary with nothing preloaded drains like any finished track.
func TestQueueEndEvent(t *testing.T) {
	...
	// loadLast records last as the committed source, as a completed start would.
	loadLast := func(_ *testing.T, m *Model) { m.playing = m.capturePlaybackTrack(last, 1) }
	tests := []struct {
		name   string
	...
	}{
		{
			...
			player: &playbackFakeEngine{playing: true, drained: true, currentTicket: 1},
		...
		},
		{
			...
			player: &playbackFakeEngine{playing: true, currentTicket: 1},
			tracks: []playlist.Track{first, last},
			load:   loadLast,
		...
		},
		{
			...
			tracks: []playlist.Track{last},
			load: func(t *testing.T, m *Model) {
				completeQueueEndSource(t, m, m.playTrack(last))
				if !errors.Is(m.err, startErr) {
					t.Fatalf("playback error = %v, want %v", m.err, startErr)
				}
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertQueueEndTrack(t, messages, last)
		})
	}
}

// A replacement that fails leaves the previous track playing; running out of
// queue afterwards reports that track, once, whether skipped or drained.
	...
	for _, path := range []string{"/music/b.flac", "https://example.com/b.mp3", "https://www.youtube.com/watch?v=b"} {
		for _, ending := range []string{"next", "paused next", "drain"} {
			t.Run(path+"/"+ending, func(t *testing.T) {
				...
				pl.SetIndex(1)
				cmd := m.playTrack(b)
				if needsAsyncPrepare(b) {
					completeQueueEndSource(t, &m, cmd)
				}
				if !errors.Is(m.err, engine.startErr) {
					t.Fatalf("start error = %v, want %v", m.err, engine.startErr)
				}
				if m.playing == nil || m.playing.track.Path != a.Path || !engine.IsPlaying() {
					t.Fatalf("failed B lost A: playing=%+v engine playing=%t", m.playing, engine.IsPlaying())
				}
...
// A source still loading when the queue runs out never started, so nothing
// finished; its late preparation result must not revive it either.
...
func TestQueueEndStaleStreamAfterFailedReplacementPreservesTrack(t *testing.T) {
	...
	completeQueueEndSource(t, &m, m.playTrack(b))
	completeQueueEndSource(t, &m, stale)
	if m.playing == nil || m.playing.track.Path != a.Path || !errors.Is(m.err, engine.startErr) {
		t.Fatalf("stale result changed retained playback: playing=%+v err=%v", m.playing, m.err)
	}
	updated, _ := m.Update(playback.NextMsg{})
	m = updated.(Model)
	m.Update(playback.NextMsg{})
...
// A replacement that was prepared but never committed did not start, so
// running out of queue reports the track that was still playing.
func TestQueueEndReportsPlayingTrackOverUncommittedReplacement(t *testing.T) {
	for _, replacement := range []string{"", "/music/c.flac", "https://example.com/c.mp3"} {
		t.Run("replacement="+replacement, func(t *testing.T) {
			...
			m := Model{player: engine, playlist: pl, luaMgr: mgr}
			m.playTrack(a)
			// B is prepared, but its message has not reached the model.
			msg := m.nextTrack()()
			if replacement != "" {
				...
				if m.playing == nil || m.playing.track.Path != a.Path || !errors.Is(m.err, engine.startErr) {
					t.Fatalf("failed C lost A: playing=%+v err=%v", m.playing, m.err)
				}
			}
			m.nextTrack()
			updated, _ := m.Update(msg)
...
func completeQueueEndSource(t *testing.T, m *Model, cmd tea.Cmd) {
	t.Helper()
	if cmd == nil {
	...
	if !ok {
		t.Fatal("prepare command did not return sourcePreparedMsg")
	}
	updated, _ := m.Update(msg)
	*m = updated.(Model)
}
