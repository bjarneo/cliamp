...
// ArtistDetailLoader contract, using the shared fakeSpotifyLibProvider plumbing.

// ArtistDetail makes fakeSpotifyLibProvider implement provider.ArtistDetailLoader
...
func (f *fakeSpotifyLibProvider) ArtistDetail(artistID string) (provider.ArtistDetail, error) {
...
func newArtistTestModel(fake *fakeSpotifyLibProvider) Model {
...
func TestArtistScreenOpenFromNavBrowserArtist(t *testing.T) {
	fake := &fakeSpotifyLibProvider{name: "Spotify", artistsList: []provider.ArtistInfo{artistFixtureArtist()}}
	fake.artistDetails = map[string]provider.ArtistDetail{"ar1": artistDetailFixture()}
	m := newArtistTestModel(fake)
