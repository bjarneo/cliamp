...
// contracts, using the shared fakeSpotifyLibProvider plumbing. The homeFake
...
func homeFixture() *homeFakeProvider {
	return &homeFakeProvider{
		fakeSpotifyLibProvider: &fakeSpotifyLibProvider{
			name: "Spotify",
...
func TestHomeQueueLoadCancelsContentPaging(t *testing.T) {
	...
	m = updated.(Model)
	if !m.home.content.paging.active || m.home.content.paging.next != 2 {
		t.Fatalf("paging = %+v; want active chain with next at 2", m.home.content.paging)
	}
