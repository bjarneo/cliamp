...
// recordingSaver captures configSaver.Save calls.
type recordingSaver struct {
	pairs [][2]string
}

func (r *recordingSaver) Save(key, value string) error {
	r.pairs = append(r.pairs, [2]string{key, value})
	return nil
}

func (r *recordingSaver) has(key, value string) bool {
...
func newSmartTestModel(fake *fakeRecommenderProvider, saver *recordingSaver) Model {
...
func TestSmartToggleNeedsRecommender(t *testing.T) {
	fake := &fakeSpotifyProvider{name: "Spotify"} // implements CustomStreamer, not Recommender
