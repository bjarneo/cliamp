...
func buildApp() *cli.Command {
	rootFlags := []cli.Flag{
		...
		&cli.StringFlag{Name: "provider", Usage: "default provider: radio, navidrome, plex, jellyfin, emby, spotify, yt, youtube, ytmusic"},
...
func overridesFromFlags(c *cli.Command) (config.Overrides, error) {
	...
	if c.IsSet("provider") {
		...
		switch v {
		case "radio", "navidrome", "spotify", "plex", "jellyfin", "emby", "yt", "youtube", "ytmusic":
			ov.Provider = &v
		default:
			return ov, fmt.Errorf("--provider must be radio, navidrome, spotify, plex, jellyfin, emby, yt, youtube, or ytmusic (got %q)", v)
		}
	}
